...
We use an external party, UptimeRobot, to verifiably measure our uptime and response time. We publish these on our public status page at [status.mastodon.site](https://status.mastodon.site).
...
Although the public status page only shows our own mastodon server ([social.mastodon.site](https://social.mastodon.site)), the same check is set up for every customer's server and its results can be requested by [contacting us](/support/overview).
