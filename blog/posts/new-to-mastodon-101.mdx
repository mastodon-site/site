...
If a user on the same server as you, it should be super straight fowrard. You can click follow from their profile directly, or search for them and add them. Magic.

If the user is on a different server, you'll need to search their full username (the `username` bit and the `@server.com` bit) in your home server. You should see them and be able to add them in the same way as local users.
